...
export const EnrollmentController = {
  ...
  async requestEnrollment(req, res) {
    try {
      ...
      const result = await EnrollmentService.requestEnrollment(userId, courseId);
      return res.json(result);
    } catch (err) {
      console.error('[EnrollmentController] requestEnrollment error:', err);
      if (err && err.status === 404) return res.status(404).json({ error: 'course not found' });
      return res.status(500).json({
        error: 'Internal server error',
        details: err && err.message ? err.message : 'unknown error',
        code: err && err.code ? err.code : null
  ...
  async canEnroll(req, res) {
    try {
      ...
      const result = await EnrollmentService.checkPrerequisites(userId, courseId);
